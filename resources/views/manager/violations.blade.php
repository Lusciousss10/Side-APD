        ...
        <div class="container mx-auto px-4">
                            ...
                            <tr class="text-gray-200">
                                ...
                                <td class="py-2 px-4 border-b border-gray-600">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('manager.download', $violation->filename) }}" class="bg-blue-600 text-white px-4 py-2 rounded">Download</a>
            ...
            <div class="mt-8 text-center">
                <form action="{{ route('manager.deleteAll') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete all items?');">
                    @csrf
                    @method('DELETE')
                    <!-- Invisible and small container -->
                    <div class="inline-block overflow-hidden w-16 h-8">
                        <button type="submit" class="bg-red-600 text-white w-full h-full text-xs rounded">Delete All</button>
                    </div>
                </form>
            </div>            
        </div>
