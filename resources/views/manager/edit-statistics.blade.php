                    ...
                    <div class="container mx-auto">
                        <div class="bg-gray-800 border border-gray-700 rounded-lg shadow">
                            <div class="bg-gray-700 px-4 py-3 border-b border-gray-600 rounded-t-lg flex justify-between items-center">
                                <h3 class="text-lg font-medium text-gray-100">Edit Statistik Pelanggaran</h3>
                            </div>
                            <div class="p-4">
                                @if (session('success'))
                                ...
                                <form action="{{ route('manager.statistics.update') }}" method="POST" class="space-y-6">
                                    @csrf
                                    <div>
                                        <label for="jumlah_meninggal" class="block text-sm font-medium text-gray-100">Jumlah Orang Meninggal</label>
                                        <input type="number" id="jumlah_meninggal" name="jumlah_meninggal" value="{{ old('jumlah_meninggal', $violationstatistic->jumlah_meninggal) }}" class="mt-1 block w-full border border-gray-600 bg-gray-700 text-gray-100 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                        @error('jumlah_meninggal')
                                            <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="jumlah_kasus_ditindaklanjuti" class="block text-sm font-medium text-gray-100">Jumlah Kasus Ditindaklanjuti</label>
                                        <input type="number" id="jumlah_kasus_ditindaklanjuti" name="jumlah_kasus_ditindaklanjuti" value="{{ old('jumlah_kasus_ditindaklanjuti', $violationstatistic->jumlah_kasus_ditindaklanjuti) }}" class="mt-1 block w-full border border-gray-600 bg-gray-700 text-gray-100 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
